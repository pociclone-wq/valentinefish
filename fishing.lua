local Page = ... -- Menangkap AllPages["Fishing"] dari script utama
...
-- [[ UI SETUP ]] --
-- Bersihkan Page dulu agar tidak double jika script dipanggil ulang
for _, v in pairs(Page:GetChildren()) do
    if v:IsA("Frame") or v:IsA("TextButton") or v:IsA("TextBox") or v:IsA("UIListLayout") then
        v:Destroy()
    end
end
...
Page.CanvasSize = UDim2.new(0, 0, 0, 400) -- Memastikan bisa di-scroll
...
local function CreateToggle(text, key, order)
    ...
    TogBg.MouseButton1Click:Connect(function()
        ...
        if key == "FishingActive" and active then
            task.spawn(function()
                while Settings.FishingActive do
                    pcall(function()
                        ...
                        NetPath:WaitForChild("RF/"..RF_Cast):InvokeServer(unpack(args))
